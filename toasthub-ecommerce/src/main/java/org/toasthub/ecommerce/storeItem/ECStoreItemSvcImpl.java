...
@Service("ECStoreItemSvc")
public class ECStoreItemSvcImpl implements ECStoreItemSvc, ServiceProcessor {

	@Autowired
	@Qualifier("ECStoreItemDao")
	ECStoreItemDao storeDao;
	...
	public void process(RestRequest request, RestResponse response) {
		...
		case "SAVE":
			if (!request.containsParam(PrefCacheUtil.PREFFORMKEYS)) {
				List<String> forms =  new ArrayList<String>(Arrays.asList("EC_STOREITEM_FORM"));
	...
	public void save(RestRequest request, RestResponse response) {
		try {
			...
			if (inputList.containsKey(GlobalConstant.ITEMID) && inputList.get(GlobalConstant.ITEMID) != null && !"".equals(inputList.get(GlobalConstant.ITEMID))) {
				...
				ECStoreItem storeItem = new ECStoreItem();
	...
	public void deleteAttachment(RestRequest request, RestResponse response) {
		try {
			storeDao.deleteAttachment(request.getParamLong("ATTACHMENT_ID"));

...
public class ECPurchaseSvcImpl implements ECPurchaseSvc {
	...
	/*	if (request.containsParam("PURCHASE_ITEM_ID")) {
			try {
				PurchaseRequest req = purchaseDao.approve(new Long((int) request.getParam("PURCHASE_ITEM_ID")));
	...
	public void deny(RestRequest request, RestResponse response) {
		/*if (request.containsParam("PURCHASE_ITEM_ID")) {
			try {
				PurchaseRequest req = purchaseDao.deny(new Long((int) request.getParam("PURCHASE_ITEM_ID")));
